...
We may also want to verify that if the user enters the sas with an invald card, he will not make it to the other side, but if he has a valid card, he may.

This may be expressed as follows: \verb|A[] ( (userCard == 0 && User.in_sas) imply 

Sadly, the syntax analyzer of UppAal does not accept a quantifier after the keyword \texttt{imply}.
...
%\begin{figure}[!h]
%	\centering
%    \includegraphics[scale=0.1]{photo.JPG}
%    \caption{Human trials. Take 1. Entering the twilight zone.}
%    \label{inside}
%\end{figure}
...
\section{Future work}
%*Faiblesses : 
%	*Un user pourrait entrer par la porte 2 quand elle s'ouvre.
%	*Quand l'alarme se déclenche, un nouvel utilisateur peut entrer dans le sas par la porte 1 et stoper l'alarme. Au final, on peut se retrouver avec un nombre pair de users dans le sas.
%		->On pourrait supposer que le sas a la place pour une seule personne.
%		->Il faudrait pouvoir voir le sens de passage des utilisateurs dans la porte 1
%			->Nécessite un second capteur infrarouge.

%Expliquer ce qu'on pourrait faire pour améliorer notre système.
...
%%% TODO
%*Do the link between the strategies and the code 
%*Stratégie de contrôle avec Until. Ben oui, parce qu'une stratégie de victoire serait de ne jamais ouvrir la première porte.
%	>Mais non, c'est débile, on commence à gateConf == 1. De toute façon, quoi qu'on fasse, ce connard d'environnement pourrait
%		tout simplement décider de ne jamais passer devant l'infra1 et donc la gateConf ne changerait jamais.
%		Donc on le laisse comme et c'est bon, hein.
%*Faire meilleur lien entre différents cas physiques et modèles.
%*Modélisation de la carte (au moins en discuter dans le rapport). Process card (uppaal tiga)
%	>Les limitations du programme ne permettent pas de faire ce qu'on veut.
%		Je voudrais écrire A[] (userCard == 0 imply ( not(User.after_sas) U user.before_sas) )
%		i.e. dans tous les états, si userCard == 0, on ne peut pas atteindre after_sas et on doit retourner dans before_sas.
%*Rapport : Expliquer à haut niveau ce que le projet devrait faire -> expliquer propriétés.
